...
export default function SlideViewer() {
  ...
  const { connected, startQuiz } = useQuizSync();
  ...
  return <CourseShell stage={1}>
    {!connected && <p role="status">수업 서버에 연결 중입니다. 연결되면 강사의 퀴즈 시작에 맞춰 이동합니다.</p>}
    ...
    <footer className="finance-controls">
      ...
      {index < pages.length-1 ? <button onClick={() => move(index+1)}>다음 슬라이드</button> : isAdmin ? <button className="primary" disabled={!connected || starting} onClick={beginQuiz}>{starting ? '퀴즈 시작 중…' : '퀴즈로 이동'}</button> : <span>강사가 퀴즈를 시작할 때까지 기다려 주세요.</span>}
    </footer>
