...
import { lessonSteps } from '../../contents/financial-course.js';
...
export default function RoundIndicator() {
  ...
  return <div className="round-indicator-page"><PageHeader title={`${round}차시`}/>
    <main className="round-indicator">
      ...
      <div className="round-step-container">{lessonSteps(round).map(({label,step:stage},index)=><div key={label} className={step===stage?'current-step':'step-indicator'} aria-current={step===stage?'step':undefined}><h4>STEP {index+1}</h4>{label}</div>)}</div>
