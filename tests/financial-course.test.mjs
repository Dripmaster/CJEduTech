...
test('next lesson resets the video index and ends only after lesson four', () => {
  assert.deepEqual(nextLesson(1), { round: 2, step: 1, videoId: 0, final: false });
  assert.deepEqual(nextLesson(3), { round: 4, step: 1, videoId: 2, final: false });
...
test('restore ignores legacy session positions and derives video from lesson', () => {
  assert.deepEqual(restoreProgress(null), { round: 1, step: 1, videoId: null });
  assert.deepEqual(restoreProgress('{"round":4,"step":3,"videoId":9}'), { round: 4, step: 3, videoId: 2 });
  assert.deepEqual(restoreProgress('{"round":8,"step":3}'), { round: 1, step: 1, videoId: null });
  assert.deepEqual(restoreProgress('broken'), { round: 1, step: 1, videoId: null });
});
test('theory preserves page order while moving quizzes to their own phase', () => {
  ...
  assert.equal(getLesson(4).theoryPages.at(-1),63);
});
...
test('saved quiz positions in lessons three and four resume at the video', () => {
  for (const round of [3,4]) assert.deepEqual(restoreProgress(JSON.stringify({round,step:2})), {round,step:3,videoId:round===3?1:2});
...
test('reconnects catch up skipped-quiz lessons without rewinding completed stages', () => {
  ...
  assert.deepEqual(lessonSteps(1).map(s=>s.step),[1,2]);
});
